...
class LobbyWindow implements ActionListener, ItemListener
{
	...
	public void actionPerformed(ActionEvent event)
	{
		if(event.getSource() == JB_Connect || event.getSource() == JTF_IPAddr) {
			...
			myNetworkType.myRoomWnd = new RoomWindow((JPanel) motherPane.getContentPane(), myNetworkType);	//룸윈도우 생성하고 네트워크와 연결.
			
			JPanel_Lobby.setVisible(false);
			myNetworkType.myRoomWnd.JPanel_Room.setVisible(true);
			
			if(myNetworkType.isServer())	myNetworkType.SendChatMsg("서버를 개설하였습니다,");
			else							myNetworkType.SendChatMsg("접속하였습니다.");
		}
	...
	public void itemStateChanged(ItemEvent event) {		// Server 항목에 체크할 경우 IP 입력필드가 비활성화됨.
